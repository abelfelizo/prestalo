...
import { calcularPrestamo, primeraFechaPago, fmt } from '@/lib/calculos'
...
export default function NuevoPrestamo() {
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={{ padding: 20, paddingTop: 56 }}>
      ...
      <View style={s.chips}>
        {FRECUENCIAS.map((f) => (
          <TouchableOpacity key={f} style={[s.chip, frecuencia === f && s.chipSel]} onPress={() => setFrecuencia(f)}>
            <Text style={[s.chipText, frecuencia === f && s.chipTextSel]}>{f}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {n > 0 && cap > 0 && (
        <View style={s.preview}>
          ...
          <View style={s.previewRow}><Text style={s.previewLabel}>Total a cobrar</Text><Text style={s.previewVal}>{fmt(resumen.montoTotal)}</Text></View>
          <View style={s.previewRow}><Text style={s.previewLabel}>Interés</Text><Text style={s.previewVal}>{fmt(resumen.interesTotal)}</Text></View>
          <View style={s.previewRow}><Text style={s.previewLabel}>Cuota ({frecuencia})</Text><Text style={[s.previewVal, { color: COLORS.gold }]}>{fmt(resumen.cuota)}</Text></View>
        </View>
      )}
